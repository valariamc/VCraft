...
minecraftversion=$(cat "$workdir/../Paper/work/BuildData/info.json"  | grep minecraftVersion | cut -d '"' -f 4)
...
acrafttestdir="${ACRAFT_TEST_DIR:-$workdir/test-server}"

mkdir -p "$acrafttestdir"
cd "$acrafttestdir"
...
folder="$basedir/AlphheimCraft-Server"
jar="$folder/target/AlphheimCraft-${minecraftversion}.jar"
if [ ! -d "$folder" ]; then
(
...
if [ ! -z "$ACRAFT_TEST_COMMAND_WRAPPER" ]; then
    $ACRAFT_TEST_COMMAND_WRAPPER $cmd
else
